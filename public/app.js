...
const PROVIDERS = [
  { id: 'dashscope', name: '阿里百炼 · 通用', baseUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1', model: 'qwen3.6-flash', direct: true },
  { id: 'mimo', name: '小米 MiMo · 官方', baseUrl: 'https://api.xiaomimimo.com/v1', model: 'mimo-v2.5-pro', direct: true },
  { id: 'mimo-tp-cn', name: '小米 Token Plan · 国内', baseUrl: 'https://token-plan-cn.xiaomimimo.com/v1', model: 'mimo-v2.5-pro', direct: true },
  { id: 'mimo-tp-sg', name: '小米 Token Plan · 新加坡', baseUrl: 'https://token-plan-sgp.xiaomimimo.com/v1', model: 'mimo-v2.5-pro', direct: true },
  { id: 'ali-tp-cn', name: '阿里 Token Plan · 北京（网页直连受限）', baseUrl: 'https://token-plan.cn-beijing.maas.aliyuncs.com/compatible-mode/v1', model: 'qwen3.6-flash', direct: false },
  { id: 'ali-tp-sg', name: '阿里 Token Plan · 新加坡（网页直连受限）', baseUrl: 'https://token-plan.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1', model: 'qwen3.6-flash', direct: false },
  { id: 'custom', name: '自定义（OpenAI 兼容）', baseUrl: '', model: '', direct: true },
];
...
const KEY_META_DEFAULT = 'Key 只存在这台设备的浏览器里（localStorage），不会上传到任何服务器，也不会进代码仓库。';

/** 有些网关在 CORS 预检阶段就要认证（返回 401 无 CORS 头），浏览器直连必然失败，提前说清楚 */
function updateProviderHint() {
  const p = currentProvider();
  if (p && p.direct === false) {
    els.keyMeta.innerHTML =
      '<b class="warn">这个网关不支持网页直接调用</b>（预检阶段就要认证，浏览器会被拦）。' +
      '两条路：改用「阿里百炼 · 通用」或小米网关；或者开着电脑、把识别引擎切到本地，让电脑帮你转发。';
    return;
  }
  const c = cloud.load();
  els.keyMeta.textContent = c.apiKey ? 'Key 已保存在本机浏览器（只显示状态，不回显内容）。' : KEY_META_DEFAULT;
}
...
$('btnTestKey').addEventListener('click', async () => {
  ...
  try {
    ...
    const p = currentProvider();
    const extra = p && p.direct === false ? '｜该网关已知不支持网页直连，换「阿里百炼 · 通用」或小米网关试试' : '';
    els.keyMeta.textContent = '失败：' + (e.name === 'AbortError' ? '超时' : errText('NETWORK')) + extra + '｜' + String(e.message || '').slice(0, 120);
  } finally {
    btn.disabled = false;
